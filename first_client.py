...
agilent_value = None  # Глобальная переменная для хранения значения
agilent_value_event = threading.Event()  # Событие для ожидания нового значения
    ...
    while not agilent_stop_event.is_set():
        with agilent_lock:
            ...
            if value is not None:
                with ustavka_lock:
                    agilent_value = value  # Обновляем значение
                    agilent_value_event.set()  # Сообщаем, что значение обновлено
            else:
                print("Ошибка: Agilent не вернул значение")
        ...
        for current_IDT in range(0, 11):
            for current_ustavka_IDT in range(990, 1205, 5):
                ...
                with ustavka_lock:
                    ...
                    current_agilent_value = agilent_value  # Копируем значение
                    agilent_value_event.clear()  # Сбрасываем событие
                    
                if param_value is not None and current_agilent_value is not None:
                    write_to_csv(current_IDT, current_ustavka_IDT, param_value, current_agilent_value)
